...
# CODE TO EXPLORE MULTIPLE PROPERTIES OF THE GLV MODEL WITHIN THE MU-SIGMA PHASE SPACE WITH MIGRATION
    ...
    while i<frac:
        ...
        for j in range(0,reps): # DIFFERENT SIMULATIONS AT EACH MU,SIGMA LOCATION
            ...
            def run(S, tmax=temps1, EPS=EPS,**kwargs):
                def eqs(t,x):
                    dx = x*(1+np.dot(A,x))
                    dx[x<=EPS]=  np.maximum(0,dx[x<=EPS])  #if a species is below threshold, abundance cannot keep decreasing
                    return dx
                ...
                if surviving > 2:
                    ...
                    for spp1 in range(Sprime):
                        for spp2 in range(spp1 + 1, Sprime):  # Iterate over unique pairs
                            ...
                            sol = solve_ivp(eqs, [0, 1000], x0, method='RK45')
...
fig.tight_layout()
plt.savefig("GLV_heatmaps_FigureS3A.png", format='png')
plt.close()
