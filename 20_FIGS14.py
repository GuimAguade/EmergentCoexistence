...
# CODE TO EXPLORE THE LAIRD-SCHAMP INDEX IN COMMUNITIES WITH EMERGENT COEXISTENCE
...
# DEFINE A
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0: # INCORPORATE THIS LAST CONDITION TO EXPLORE THE LRE STATISTICS IN COMMUNITIES WITH MORE THAN 3 EXCLUSIONS AS DISCUSSED IN THE ESM.
            
                # THE SYSTEM IS STABLE: ESTIMATE THE LAIRD SCHAMP INDEX:
                
                # HOW MANY EXCLUSIONS FOR EACH SPECIES?
                count_wins = np.sum(Aprime <-1, axis=0)
                
                # Given values
                total_sum = np.sum(count_wins)  # Total number to distribute
                vector_length = len(count_wins)  # Number of positions
                # Homogeneous Distribution
                base_value = total_sum // vector_length
                remainder = total_sum % vector_length
                
                # GENERATE THE MOST HOMOGENEOUS VECTOR OF WINS POSSIBLE WHERE ALL ELEMENTS ARE BASE VALUE
                homogeneous = np.full(vector_length, base_value)
                homogeneous[:remainder] += 1  # Distribute remainder evenly
                
                # GENERATE THE MOST HETEROGENEOUS VECTOR OF WINS POSSIBLE AS A DECREASING DISTRIBUTION
                heterogeneous = np.arange(vector_length - 1, -1, -1)  # [n-1, n-2, ..., 0]
                # Scale to match total_sum
                heterogeneous = (heterogeneous / np.sum(heterogeneous) * total_sum).astype(int)
                # Adjust for rounding errors
                difference = total_sum - np.sum(heterogeneous)
                heterogeneous[:difference] += 1
                
                s_obs = np.std(count_wins)
                s_max = np.std(heterogeneous)
                s_min = np.std(homogeneous)
                ...
                if s_obs > s_max:
                    ...
                    RPSprime.append( s_obs/s_max)# ( s_max - s_obs  ) / (s_max - s_min)) # HERE IS WHERE THE CORRECTION HAS TO BE IMPLEMENTED, s_obs / s_max
                
                count_wins = np.sum(shuffle_off_diagonal(Aprime) <-1, axis=0)
                ...
                if s_obs > s_max:
                    ...
                    RPSshuff.append( s_obs/s_max)#  ( s_max - s_obs  ) / (s_max - s_min))
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0:
                ...
                # SAME MECHANISM AS ABOVE TO ESTABLISH MOST HOMOGENEOUS AND HETEROGENEOUS VECTORS OF WINS
                ...
                if s_obs > s_max:
                    ...
                    RPSnested.append( s_obs/s_max)# ( s_max - s_obs  ) / (s_max - s_min))
    ...
    for j in range(reps):
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0: # INCORPORATE THIS LAST CONDITION TO EXPLORE THE LRE STATISTICS IN COMMUNITIES WITH MORE THAN 3 EXCLUSIONS AS DISCUSSED IN THE ESM.
                ...
                homogeneous = np.full(vector_length, base_value)
                homogeneous[:remainder] += 1  # Distribute remainder evenly
                # Generate decreasing distribution: count, count-1, ..., 0
                heterogeneous = np.arange(vector_length - 1, -1, -1)  # [n-1, n-2, ..., 0]
                # Scale to match total_sum
                heterogeneous = (heterogeneous / np.sum(heterogeneous) * total_sum).astype(int)
                # Adjust for rounding errors
                ...
                #print(count_wins, heterogeneous, homogeneous)
                ...
                if s_obs > s_max:
                    ...
                    RPScross.append(s_obs/s_max)# ( s_max - s_obs  ) / (s_max - s_min))
...
plt.savefig(f"FigS14.png", format='png', dpi=200)
plt.show()
