...
# CODE TO EXPLORE MULTIPLE PROPERTIES OF A SUBLINEAR GROWTH MODEL
...
EPS=10.**-20 
EXT=0.03 #Threshold of extinction
    ...
    while i<frac:
        ...
        for j in range(0,reps): # DIFFERENT SIMULATIONS AT EACH MU,SIGMA LOCATION
            ...
            def run(S, tmax=temps1, EPS=EPS,**kwargs):
                def eqs(t,x):
                    dx = (x**(0.75))-(d*x) + x*np.dot(A,x)
                    dx[x<=EXT]=0#  np.maximum(0,dx[x<=EPS])  #if a species is below threshold, abundance cannot keep decreasing
                    x[x<=EXT]=0
                    return dx
            ...
            if diff > 0: # DIFFERENT STATE, NOT STATIONARY
                ...
                if surviving > 2:
                    ...
                    def eqs(t, x):
                        dx = (x**(0.75))-(d*x) + x*np.dot(Aprime,x)
                        dx[x<=EXT]=0#  np.maximum(0,dx[x<=EPS])  #if a species is below threshold, abundance cannot keep decreasing
                        x[x<=EXT]=0
                        return dx
...
fig.tight_layout()
plt.savefig("GLV_heatmaps_FigureS3D.png", format='png')
plt.close()
