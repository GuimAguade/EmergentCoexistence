...
# CODE TO EXPLORE MULTIPLE PROPERTIES OF THE A MULTILAYER MODEL WITH COOPERATIVE AND COMPETITIVE INTERACTIONS
...
while z < frac:
    ...
    # DEFINE AN INCREASING VALUE FOR COMPETITION STRENGTH AND STORE
    meanB = (z*(meanBmax)/float(frac)) + 0.000001
    B=np.random.lognormal(mean=np.log(meanB), sigma=np.log(std), size=(SP,SP)) 
    varAlist.append(np.mean(B))
    ...
    while i<frac:
        ...
        meanA= (i*(meanAmax)/float(frac)) + 0.000001
        A=np.random.lognormal(mean=np.log(meanA), sigma=np.log(std), size=(SP,SP)) #COOP matrix, all interactions uniform
        if z==0:
        ...
        for j in range(0,reps): # DIFFERENT SIMULATIONS AT EACH MU,SIGMA LOCATION
            ...
            def run(S, tmax=temps1, EPS=EPS,**kwargs):
                def eqs(t,x):
                    ...
                    dx[x<=EPS]=0#  np.maximum(0,dx[x<=EPS])  #if a species is below threshold, abundance cannot keep decreasing
                    x[x<=EPS]=0
                    return dx
            ...
            if diff > 0: # DIFFERENT STATE, NOT STATIONARY
                ...
                if surviving > 2:
                    ...
                    # 1. BUILD THE TOURNAMENT MATRIX OF THE SURVIVING SPECIES
                    ...
                    extinct = np.where(np.array(xzeros) == 0)[0]
                    Aprime = np.delete(np.delete(A,extinct,axis=0),extinct,axis=1)
                    Bprime = np.delete(np.delete(B,extinct,axis=0),extinct,axis=1)
                    
                    Sprime = surviving
                    
                    def eqs(t, x):
                        dx = (x * np.dot(Aprime, x / (np.mean(g) + x))) - (np.mean(d) * x) - (x * np.dot(Bprime, x))
                        dx[x <= EPS] = 0  # Prevent further decrease of near-extinct species
                        x[x <= EPS] = 0
                        return dx

                    found_extinction = False  # Flag to track if EC occurs
                    
                    for spp1 in range(Sprime):
                        for spp2 in range(spp1 + 1, Sprime):  # Iterate over unique pairs
                            x0 = np.zeros(Sprime)  # Start with all species at zero abundance
                            x0[spp1] = 1  # Set initial condition for species i
                            x0[spp2] = 1  # Set initial condition for species j

                            sol = solve_ivp(eqs, [0, 3000], x0, method='RK45')
                            final_state = sol.y[:, -1]  # Get final species abundances

                            # Determine coexistence outcome
                            if final_state[spp1] < 10*EPS or final_state[spp2] < 10*EPS:
                                Presence_onesprime[z, i] += 1
                                found_extinction = True  # Set flag to true
...
fig.tight_layout()
plt.savefig("GLV_heatmaps_FigureS3C.png", format='png')
plt.close()
