...
# CODE TO SIMULATE AN EMPIRICAL TEST IN WHICH WE TRY TO INFER COMPOSITION FROM PAIRWISE COEXISTENCE
...
for i in range(frac):
    ...
    for j in range(reps):
        ...
        # ARE ALL PAIRS COEXISTING PAIRS IN THE SUBSET WE HAVE CHOSEN?
        
        if np.all(Aprime >= -1):
            
            # OK, THE SPECIES WE HAVE CHOSEN ARE PAIRWISE COEXISTORS. BUT, DO THEY COEXIST IN THE COMMUNITY?
            ...
            # THEY DON'T COEXIST IN THE FULL COMMUNITY, SOME SPECIES HAS NEGATIVE ABUNDANCE
            if np.any(row_sums <= 0):
                Coexistence.append(0)
            else:
            ...
            # MEASURE SPECTRAL RADIUS, DOES IT INFORM ABOUT PREDICTABILITY SUCCESS?
            np.fill_diagonal(Aprime, np.zeros(len(Aprime)))
            eigenvalues = np.linalg.eigvals(Aprime)  # Compute eigenvalues
            Spectral.append(max(abs(eigenvalues))) 
   
   
# GENERATE DIFFERENT VISUALIZATIONS TO STUDY HOW THE SUCCESS OF PREDICTING COEXISTENCE IS RELATED TO THE SPECTRAL RADIUS OF THE SYSTEM   
...
plt.savefig("Figure_S9A_spectral_radius.png", format='png')
