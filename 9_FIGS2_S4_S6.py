...
# CODE TO EXPLORE MULTIPLE PROPERTIES OF THE GLV MODEL WITHIN THE MU-SIGMA PHASE SPACE WITH DIFFERENT MATRIX RANDOMIZATIONS
...
while z < frac:
    ...
    while i<frac:
        ...
        for j in range(0,reps): # DIFFERENT SIMULATIONS AT EACH MU,SIGMA LOCATION
            ...
            # HERE IS WHERE THE RANDOMIZATION HAS TO BE IMPLEMENTED, E.G.
            
            # NESTEDNESS
            Nestedness = 0.75
            for row in range(S):
                for col in range(row+1,S):
            ...
            #ANTISYMMETRY = 1
            #for row in range(S):
            #    for col in range(row+1S):
            #        if np.random.rand() < ANTISYMMETRY
            #        A[row,col] = -A[col,row]
            
            # SINGLE-RESOURCE + CROSS-FEEDING
...
fig.tight_layout()
plt.savefig("GLV_heatmaps_FigureS2.png", format='png')
plt.close()
