...
# CODE TO EXPLORE THE FRACTION OF ROCK-PAPER-SCISSORS MOTIFS IN COMMUNITIES WITH EMERGENT COEXISTENCE
...
# COUNT THE NUMBER OF STRUCTURES IN WHICH THREE SPECIES ARE CONNECTED BY EXCLUSION, NOT NECESSARILY RPS
def measure_triplets(A):
    num_species = A.shape[0]
    excluding_triplets = 0
   
    # Iterate over all distinct triplets (i, j, k) with i, j, k all different
    for i in range(num_species):
        for j in range(i+1, num_species):
            for k in range(j+1, num_species):
                if i != j and j != k and i != k:
                    if (A[i, j] <-1 or A[j, i] <-1) and (A[i, k] <-1 or A[k, i] <-1) and (A[j, k] <-1 or A[k, j] <-1):
                        excluding_triplets +=1
    
    return excluding_triplets
 
# COUNT THE NUMBER OF TRIPLETS THAT HAVE AN EXPLICIT RPS STRUCTURE    
def measure_RPS(A):
    num_species = A.shape[0]
    excluding_triplets = 0
    rps_triplets = 0
    ...
    for i in range(num_species):
        for j in range(i+1, num_species):
            for k in range(j+1, num_species):
                if i != j and j != k and i != k:
                    if (A[i, j]<-1 or A[j, i]<-1) and (A[i, k] <-1 or A[k, i] <-1) and (A[j, k] <-1 or A[k, j] <-1):
                        if A[i, j] <-1 and A[j,i]>-1 and A[k, i] <-1 and A[i,k] >-1 and A[j, k] <-1 and A[k,j] >-1:
                            rps_triplets += 1
                        elif A[i, j] >-1 and A[j,i]<-1 and A[k, i] >-1 and A[i,k] <-1 and A[j, k] >-1 and A[k,j]<-1:
                            rps_triplets += 1    
...
# DEFINE A
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0:
                ...
                Sprime.append(S)
                TRIprime.append(measure_triplets(Aprime))
                RPSprime.append(measure_RPS(Aprime))
                
                # TRANSITIVITY METRICS FOR STATES WITH SHUFFLED (RANDOMIZED) MATRIX
                Sshuff.append(S)
                TRIshuff.append(measure_triplets(shuffle_off_diagonal(Aprime)))
                RPSshuff.append(measure_RPS(shuffle_off_diagonal(Aprime)))
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0:
                ...
                Snested.append(S)
                TRInested.append(measure_triplets(Aprime))
                RPSnested.append(measure_RPS(Aprime))
    ...
    for j in range(reps):
        ...
        if np.all(row_sums > 0) and np.any(Aprime < -1):
            ...
            if np.max(np.real(np.linalg.eigvals(J))) < 0:
                ...
                Scross.append(S)
                TRIcross.append(measure_triplets(Aprime))
                RPScross.append(measure_RPS(Aprime))
...
# ADD A LINE FOR THE RANDOM EXPECTATION OF 25% RPS

plt.axhline(y=0.25, color="black", linestyle="--", linewidth=1, label="y = 0")

plt.xlabel("Species Diversity")
plt.ylabel("Mean and Std Dev of RPS fraction")
plt.savefig(f"Fig4C.png", format='png', dpi=200)
plt.show()
...
plt.savefig(f"Fig4D.png", format='png', dpi=200)
plt.show()
